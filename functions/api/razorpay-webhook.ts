// functions/api/razorpay-webhook.ts — payment.captured → plan active YA credits add + invoice
...
const PACK_CREDITS: Record<string, number> = {
  "pack-30": 9, "pack-99": 24, "pack-299": 90, "pack-899": 300,
};
...
export const onRequestPost = async (context: any) => {
  try {
    ...
    let activated = "";
    const item = String(pay.plan || "");
    const isPack = item.startsWith("pack-");

    if (isPack) {
      const credits = PACK_CREDITS[item] || 0;
      await DB.prepare("UPDATE profiles SET bonus_credits = COALESCE(bonus_credits, 0) + ? WHERE user_id = ?").bind(credits, pay.user_id).run();
      activated = item + "+" + credits + "credits";
    } else {
      await DB.prepare("UPDATE profiles SET plan = ? WHERE user_id = ?").bind(item, pay.user_id).run();
      activated = item;
    }
    ...
    if (RESEND_KEY && user) {
      ...
      const isP = isPack;
      const credits = isP ? (PACK_CREDITS[item] || 0) : 0;
      const labelRow = isP ? "Pack" : "Plan";
      const labelVal = isP ? item.toUpperCase() : item.toUpperCase();
      const successLine = isP
        ? `Your pack is TURANT active — ${credits} credits add ho gaye hain aapke account mein. 🙏`
        : `Your plan is TURANT active — studio aur gallery mein naye limits lagu hain. 🙏`;
      ...
      <div style="font-family:Arial,sans-serif;background:#f6efe6;padding:32px">
        <div style="max-width:520px;margin:auto;background:#fffdf9;border-radius:20px;padding:32px;border:1px solid #f0c674">
          ...
          <p style="text-align:center;color:#8a6d4b;font-size:13px">Namaste ${user.name || ""}, your payment is confirmed!</p>
          <table style="width:100%;margin-top:24px;border-collapse:collapse;font-size:14px">
            <tr><td style="padding:10px;border-bottom:1px solid #f0e2cc;color:#8a6d4b">${labelRow}</td><td style="padding:10px;border-bottom:1px solid #f0e2cc;font-weight:700;text-transform:uppercase">${labelVal}</td></tr>
            ${isP ? `<tr><td style="padding:10px;border-bottom:1px solid #f0e2cc;color:#8a6d4b">Credits Added</td><td style="padding:10px;border-bottom:1px solid #f0e2cc;font-weight:700">${credits} ⚡</td></tr>` : ""}
          ...
          <p style="margin-top:24px;font-size:13px;color:#8a6d4b">${successLine}</p>
      ...
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({ from: "BhaktiMedia.in <otp@bhaktimedia.in>", to: [user.email], subject: `🪔 BhaktiMedia Invoice — ${labelVal} (₹${pay.amount})`, html }),
      });
    }
